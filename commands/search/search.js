...
module.exports = class Search extends Command {
  ...
  run(message, { search_term }) {
    ...
    axios
      ...
      .then(res => {
        if (res.data.collection.metadata.total_hits == 0) return message.reply('that search term found no results')

        let img = res.data.collection.items[1].links[0].href.replace(/\s/g, '%20')
