...
client.on("message", async (message) => {
  ...
  if (message.member.roles.cache.has(helperid) || message.member.roles.cache.has(modid) || message.member.roles.cache.has(adminid) || message.member.roles.cache.has(devid) || message.author.id == ownerid) {
    if (command === "slowmode") {
      client.commands.get(command).execute(message, args, client);
      console.log(modfix + command + " was used at " + time + "By: " + message.author.tag);
    }
    if (message.member.roles.cache.has(modid) || message.member.roles.cache.has(adminid) || message.member.roles.cache.has(devid) || message.author.id == ownerid) {
      if (message.member.roles.cache.has(adminid) || message.member.roles.cache.has(devid) || message.author.id == ownerid) {
        if (message.member.roles.cache.has(devid) || message.author.id == ownerid) {
          if (command === "stats") {
            message.channel.send("I am on " + client.guilds.cache.size + " servers.")
          }
          if (command === "start") {
          ...
          if (command === "send") {
            message.delete();
            sending = message.content.slice(modfix.length + command.length + args[0].length + 2)
            client.commands.get(command).execute(message, args, client, sending, modfix, command);
            console.log(modfix + command + " was used at " + time + "By: " + message.author.tag);
          }
          if (message.author.id === ownerid && message.guild.id === main_server) {
            if (command === "eval") {
              const clean = text => {
...
client.on("message", async (message) => {
  ...
  if (command == "ping") {
    if (talkedRecently.has(message.author.id)) {
      ...
      console.log(prefix + command + " was used at " + time + "By: " + message.author.tag);
  ...
  if (command == "help") {
    if (talkedRecently.has(message.author.id)) {
      ...
      console.log("=" + command + " was used at " + time + "By: " + message.author.tag);
      //complete
      talkedRecently.add(message.author.id);
      setTimeout(() => {
        talkedRecently.delete(message.author.id);
...
client.on("message", (message) => {
  ...
  if (message.channel.type === 'dm') {
    client.channels.cache.get("788527431920123906").send("**From:** `" + message.author.tag + "(" + message.author.id + ")`\n**Message:\n**" + message.content)
  }
  ...
  if (message.member.roles.cache.has(helperid) || message.member.roles.cache.has(modid) || message.member.roles.cache.has(adminid) || message.member.roles.cache.has(devid) || message.author.id == ownerid) {
    if (command === "r") {
      sending = message.content.slice(modfix.length + command.length + args[0].length + 2)
      client.users.cache.get(args[0]).send("**From the mars bot team:** " + sending)
