  ...
  execute(message, args) {
    ...
    if (!args.length) {
      ...
      if (isNaN(amount)) {
        ...
        return message.reply(
          "There are no photos for that sol please choose a number between 1 and 5111"
        );
      }
      ...
      message.reply(
        "Unknown argument please use `=opportunity` for info about the mission and `=opportunity image (sol)` to find an search an image from a soul (sol must be a number)"
      );
  },
};
