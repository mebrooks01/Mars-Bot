...
module.exports = {
  ...
  execute(message, args) {
    ...
    if (!args.length) {
      ...
      if (!args[1]) {
        message.reply(
          "please provide a sol you want to search for\n`=spirit image (sol) (page number)`"
        );
        return;
      }
      if (!args[2]) {
        message.reply(
          "please provide a page number you want to search for\n`=spirit image (sol) (page number)`"
        );
        return;
      }
      ...
      if (isNaN(amount)) {
        return message.reply(
          "that does not seem to be a valid number. Please provide a sol to search for"
        );
      } else if (amount < 1 || amount > 2208) {
        return message.reply(
          "There are no photos for that sol please choose a number between 1 and 2208"
        );
      }
      const amount1 = parseInt(args[2]);
      if (isNaN(amount1)) {
        return message.reply(
          "that does not seem to be a valid number. Please provide a page number to look for"
        );
      }
      sol = amount;
      pgnb = amount1;
      axios
        .get(
          "https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=" +
            sol +
            "&api_key=RfChsgY4x5bxykBYjbmfyrYo7hawtVthuvietqgp"
        )
        .then((res) => {
      ...
      message.reply(
        "Unknown argument please use `=spirt` for info about the mission and `=spirt image (sol)` to find an search an image from a soul (sol must be a number)"
      );
  },
};
